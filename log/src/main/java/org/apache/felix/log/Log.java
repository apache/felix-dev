...
final class Log implements BundleListener, FrameworkListener, ServiceListener
{
        ...
        void postEvent(LogEntry entry) {
            ...
            try {
                ...
                if (logLevel == LogLevel.TRACE) {
                    ...
                    topic = "org/osgi/service/log/LogEntry/LOG_WARNING";
                } else {
                ...
                if (symbolicName != null) {
                    props.put(EventConstants.BUNDLE_SYMBOLICNAME, symbolicName);
                }
                props.put(EventConstants.BUNDLE, bundle);
                props.put("log.level", entry.getLevel());
                ...
                if (exception != null) {
                    ...
                    if (exceptionMessage != null) {
                        props.put(EventConstants.EXCEPTION_MESSAGE, exceptionMessage);
                    }
    ...
    Log(final BundleContext context, final int maxSize, final boolean storeDebug)
    {
        ...
        this.m_eventAdminTracker = new ServiceTracker<>(context, EVENT_ADMIN_CLASS,
                new EAProxyServiceTrackerCustomizer(context));
    ...
    static class EAProxyServiceTrackerCustomizer implements ServiceTrackerCustomizer<Object, EAProxy> {
        private final BundleContext m_context;

        EAProxyServiceTrackerCustomizer(BundleContext context) {
            this.m_context = context;
        }

        @Override
        public EAProxy addingService(ServiceReference<Object> reference) {
            final Object service = m_context.getService(reference);
            if (service == null) {
                return null;
            }
            try {
                return new EAProxy(reference, service);
