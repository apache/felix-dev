...
public class LogEventsTest {
    ...
    @Test
    public void testWarnEventMapping() throws InterruptedException {
        final Bundle bundle = FrameworkUtil.getBundle(LogEventsTest.class);
        final BundleContext bundleContext = bundle.getBundleContext();
        final LogEventHandler handler = new LogEventHandler();
        Dictionary<String, Object> handlerProps = new Hashtable<>(2);
        handlerProps.put(EventConstants.EVENT_TOPIC, "org/osgi/service/log/LogEntry/LOG_WARNING");
        handlerProps.put(EventConstants.EVENT_FILTER, "(bundle.symbolicName=org.apache.felix.log.itests.logevents)" );
        ServiceRegistration<?> eventHandlerRegistration =
                bundleContext.registerService(EventHandler.class.getName(), handler, handlerProps);
        try {
            ...
            Assert.assertTrue("Expected a LOG_WARNING event",
                    handler.getLatch().await(1, TimeUnit.SECONDS));
            eventHandlerRegistration.unregister();
            eventHandlerRegistration = null;
            final List<Event> events = handler.getEvents();
            ...
            Assert.assertEquals("org/osgi/service/log/LogEntry/LOG_WARNING", event.getTopic());
            Assert.assertEquals(Integer.valueOf(LogService.LOG_WARNING), event.getProperty("log.level"));
            Assert.assertEquals(LogLevel.WARN, event.getProperty("log.loglevel"));
        } finally {
            if (eventHandlerRegistration != null) {
                eventHandlerRegistration.unregister();
            }
    ...
    @Test
    public void testServiceEventsRemainAdaptedByEventAdmin() throws InterruptedException {
        final Bundle bundle = FrameworkUtil.getBundle(LogEventsTest.class);
        final BundleContext bundleContext = bundle.getBundleContext();
        final LogEventHandler handler = new LogEventHandler();
        Dictionary<String, Object> handlerProps = new Hashtable<>(2);
        handlerProps.put(EventConstants.EVENT_TOPIC, "org/osgi/framework/ServiceEvent/REGISTERED");
        handlerProps.put(EventConstants.EVENT_FILTER, "(service.objectClass=java.lang.Runnable)");
        ServiceRegistration<?> eventHandlerRegistration =
                bundleContext.registerService(EventHandler.class.getName(), handler, handlerProps);
        ServiceRegistration<?> markerRegistration = null;
        try {
            markerRegistration = bundleContext.registerService(Runnable.class.getName(), new Runnable() {
                @Override
                public void run() {
                    // Marker service used to trigger the Framework service event adapter.
                }
            }, null);
            Assert.assertTrue("Expected Event Admin to adapt the ServiceEvent",
                    handler.getLatch().await(1, TimeUnit.SECONDS));
            Assert.assertEquals("Expecting one event", 1, handler.getEvents().size());
        } finally {
            if (markerRegistration != null) {
                markerRegistration.unregister();
            }
            eventHandlerRegistration.unregister();
        }
    }
